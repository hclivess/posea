...
\texttt{nadochain.com}\quad\texttt{github.com/hclivess/nado}\\[2pt]
September 2026}}
...
countermeasures cannot price: the identity farm. Any admission rule charging a fixed cost per
identity leaves the adversary's cost-to-reward ratio equal to the honest participant's, while the
adversary alone amortises setup across its identities. On the deployment we report, some
$1{,}000$ of $1{,}191$ identities belonged to two or three operators running browser farms and
captured $42\%$ of emission, after four such rules had been tried.
...
paid per identity is a subsidy to whoever mints the most. Deployed responses have converged on
two shapes. One imposes a recurring cost or delay on each identity, which is the family we argue
below cannot work. The other binds identities to something the adversary cannot replicate---a
synchronous human ceremony in Idena's case~\cite{idena}, a physical gathering in
proof-of-personhood~\cite{pop}---which does work, at the price of a ceremony every participant
must attend.

We report a deployment in which the first family was tried four times and failed four times, give
the measurements, and argue that the recurrence is structural rather than a sequence of
implementation errors. PoSEA belongs to the second family, and substitutes a physically distinct
\emph{object} for the ceremony.
...
Table~\ref{tab:classes} lists the classes considered. Four are reachable from a web page and
differ in whether they yield a per-device handle, which decides whether condition~(5) is
enforceable at all; the fifth is the subject of \S\ref{sec:noca} and is reachable only from a
native client.
...
The third row of Table~\ref{tab:classes} is not a class any platform exposes. It is the path
constructed in \S\ref{sec:noca}, reachable only from a native client, and it binds on the
endorsement key---one per chip by manufacture, rather than one per account.
...
\emph{Notation.} $E$ is a chip's endorsement key and $N$ the object name of an attestation key
$A$ in the same chip, $N$ being a digest of $A$'s public area. $k$ is the number of challengers,
$S_i$ the secret challenger $i$ seals and $R_i$ the seed it seals under, $b_i$ the resulting
credential blob, and $H$ a collision-resistant hash. \emph{Ledger order} is the total order on
published messages that every verifier agrees on; ``strictly later'' excludes equal positions.
...
Figure~\ref{fig:protocol} shows the exchange. Nothing in Definition~\ref{def:enrol} is signed and
no key persists between enrolments: there is nothing to steal, to rotate, or to guard, and no
party holds an authority another party must trust.
