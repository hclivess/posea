"""Driving a TPM 2.0 from the prover's side: the commands an enrolment needs, and nothing else.

WHY THIS IS HERE AT ALL. Every other piece of PoSEA is a VERIFIER's concern. This is the prover's, and it is
in the reference implementation because the enrolment is worthless if a normal machine cannot actually run it.
There is no tpm2-tools dependency and no TSS: it is about two hundred lines of struct packing, which is a
smaller thing to audit than a stack, and it is the same bytes on every platform.

PREFER /dev/tpmrm0 ON LINUX. That is the kernel's resource manager: it virtualises transient handles so our
primaries and policy sessions cannot collide with another process's, and it cleans up what we leak if we
crash. /dev/tpm0 is the raw device — exclusive-open, no virtualisation, and one leaked transient handle wedges
the chip for everything else on the machine until reboot.

On Windows the same command bytes go through TBS (Tbsi_Context_Create / Tbsip_Submit_Command); the platform
crypto provider's own claim API cannot be used, because it mints a fresh per-call signer whose certificate no
verifier has ever seen — see https://github.com/hclivess/windows-tpm-claim-format for that format decoded.

THE TEMPLATES ARE NOT NEGOTIABLE. A primary key is DERIVED from its template, so one byte different yields a
different key — and for the endorsement key that means the vendor's certificate no longer matches the key the
machine holds. Reproduce the TCG profile exactly.
"""
import os
import struct
import threading
...
class Tpm:
    """A TPM reached through a character device. One command in flight at a time: the device is a
    request/response channel, and interleaving two commands on one descriptor mixes their replies.

    The COMMAND BUILDERS below are platform-independent — a TPM speaks TCG part 3 whoever hands it the bytes.
    Only `transmit` is transport. To drive a simulator or a Windows TBS handle instead, subclass and override
    it; tests/test_swtpm.py does exactly that and nothing else changes."""
