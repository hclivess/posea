"""TPM 2.0 credential protection: proving a key lives in a vendor-certified chip, with NO certificate
authority anywhere in the design.

This is the second half of PoSEA. The first half (posea/_parse.py) reads an attestation a platform already
produced. This half is for the machines where the platform produces nothing.

THE PROBLEM. A WebAuthn `tpm` statement needs `certInfo` signed by the key in `x5c[0]`, which means the chip
needs an attestation-key CERTIFICATE, which on Windows means Microsoft's AIK service. For a large share of
otherwise healthy machines that service has no authority registered for the chip's KeyId and answers HTTP 404
— confirmed service-side, with no client-side fix. On Linux there is no such service at all. In the reference
deployment this was 26.8% of every attestation attempt: a healthy TPM, and no way to use it.

Those machines are not short of evidence. They carry a vendor-signed ENDORSEMENT certificate (AMD's chains to
CN=AMDTPM, Intel's through the CSME EICAs in the chip's own NV). What is missing is one statement: *this
attestation key lives in that certified chip*.

WHY THE OBVIOUS SHORTCUTS DO NOT WORK.

  "Just send the endorsement certificate." It is public data — anyone who has seen a machine's certificate can
  copy it. And the endorsement key CANNOT SIGN: TCG defines it as a restricted decryption key, `Key Usage:
  critical, Key Encipherment`, no `digitalSignature`. It can never occupy the `x5c[0]` slot.

  "Derive the challenge from public data, so it needs no round trip." Anything every verifier can recompute,
  the prover can recompute too. The proof would prove nothing.
...
the AIK-plus-Privacy-CA model, and why the service that is failing exists at all.

THE CONSTRUCTION. A CA turns the interactive proof into a durable artifact by signing it. That works, and it
is expensive: a long-lived key able to assert any endorsement identity is a key able to mint identities, and
it has to be guarded like a mint forever. There is a cheaper way, because MakeCredential's credentialBlob is
DETERMINISTIC in (seed, name, secret) — only the OAEP-wrapped seed is randomised, and no verifier needs that
half. So a challenge issued once can be REPLAYED by everyone afterwards:
    ...
    4. every verifier recomputes the blob from (S, R), checks it equals the published blob, and checks the
       commitment is to that S

The prover can only learn S by holding the chip, and must commit BEFORE the reveal, so it cannot read the
answer off the record. Nothing is signed and no key persists: there is nothing to steal, rotate or guard.

WHAT BINDS AN IDENTITY IS THE ENDORSEMENT KEY, NEVER AN ATTESTATION KEY. A chip holds unlimited attestation
keys, so binding to one would hand a single machine an identity per enrolment. The handle is the endorsement
key's own SubjectPublicKeyInfo digest. Regenerating the endorsement seed to fake a new chip changes that key,
and the vendor's certificate no longer matches it, so it cannot enrol at all.

WHAT THIS COSTS, STATED PLAINLY. It removes the human tap: enrolment and renewal are automatic, which is
required for headless machines and also means a farm's chips cost nothing to keep running. Identity COUNT is
unchanged — one chip, one identity — but the Sybil question collapses onto the price of a chip, so WHICH ROOTS
ARE PINNED becomes the whole dial. A discrete TPM module is ~£15; a CPU-vendor fTPM implies a whole machine.
Pinning CPU vendors only is reversible in the widening direction and not the other way.

NO DEPENDENCIES, DELIBERATELY. Every primitive below — KDFa, AES-CFB, MGF1, RSA-OAEP, PKCS#1 v1.5
verification, DER — is written out. In the reference deployment this code runs inside consensus on nodes whose
runtime has no crypto library at all; in any deployment, a verifier that must agree byte-for-byte with every
other verifier is better served by an implementation it can read in one sitting than by whichever version of a
library each host happens to have installed.

See SPEC.md section 9. The ordering that makes the commit-reveal a proof is enforced in posea/enrolment.py.
"""
import hashlib
import hmac
import os
...
# NO ENDORSEMENT-CHAIN VERIFICATION LIVES HERE, and its absence is a finding rather than an omission.
#
# REAL VENDOR ENDORSEMENT CERTIFICATES ARE NOT STRICTLY DER. AMD encodes `critical: FALSE` explicitly where
# DER requires the default omitted, and python `cryptography` refuses the certificate outright:
#
#     ParseError { kind: EncodedDefault, ["Certificate::tbs_cert", "TbsCertificate::raw_extensions", 1,
#                  "Extension::critical"] }
#
# openssl and rust's x509-parser accept it. An implementer who reaches for the obvious library will conclude
# the hardware is broken; it is not. Verify the chain with a lenient parser, and — because in a replicated
# setting this is consensus input — with ONE implementation shared by every verifier, not whichever library
# each host happens to have. See SPEC.md section 9.3 for what that verification must check: the certificate
# must carry the tcg-kp-EKCertificate EKU, must not be a CA, and must be encipherment-only. A certificate
# claiming signing capability is not an endorsement key, and treating one as such would undermine the reason
# the whole handshake exists.
#
# The endorsement identity is SHA-256 over the leaf's SubjectPublicKeyInfo. pub_area_spki() below builds the
# same structure from a public area read straight out of a chip, for the case where there is no certificate
# to lift it from.
